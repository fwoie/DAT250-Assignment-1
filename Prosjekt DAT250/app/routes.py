...
def index():
    ...
    if form.login.is_submitted() and form.login.submit.data:
        ...
        if form.register.validate_on_submit():
            ...
            flash("Something went wrong with your registration")
...
def stream(username):
    ...
    if form.is_submitted():
        if form.image.data == None and form.content.data == None:
            flash('Kan ikke legge til ingenting')
        else:
            if form.image.data:
...
def profile(username):
    ...
    if form.is_submitted() and form.validate_on_submit():
        ...
        flash('Invalid input, no changes were made')
    user = query_db('SELECT * FROM Users WHERE username="{}";'.format(username), one=True)
